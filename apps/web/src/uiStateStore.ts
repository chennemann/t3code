...
export interface PersistedUiState {
  ...
  threadNeedsAttentionById?: Record<string, boolean>;
...
export function parsePersistedState(parsed: PersistedUiState): UiState {
  ...
  return {
    ...
    threadNeedsAttentionById: Object.fromEntries(
      Object.entries(sanitizeBooleanRecord(parsed.threadNeedsAttentionById)).filter(
  ...
  try {
    ...
    window.localStorage.setItem(
      ...
      JSON.stringify({
        ...
        threadNeedsAttentionById: state.threadNeedsAttentionById,
...
export function markThreadUnread(state: UiState, threadId: string): UiState {
  if (state.threadNeedsAttentionById[threadId] === true) {
    return state;
  }
  return {
    ...state,
    threadNeedsAttentionById: {
      ...state.threadNeedsAttentionById,
      [threadId]: true,
    },
  };
}
...
export function clearThreadUnread(state: UiState, threadId: string): UiState {
  ...
  const threadNeedsAttentionById = { ...state.threadNeedsAttentionById };
  delete threadNeedsAttentionById[threadId];
  return {
    ...state,
    threadNeedsAttentionById,
  };
