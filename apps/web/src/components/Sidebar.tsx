...
const SidebarThreadRow = memo(function SidebarThreadRow(props: {
  ...
  isPinned: boolean;
  // Present on cards that can be reordered: dnd-kit sortable bag applied to
  // the card root so the whole card drags (the
...
export default function Sidebar() {
  ...
  const activeThreadOrder = useUiStateStore((store) => store.activeThreadOrder);
  const activeThreadOrderAnchorById = useUiStateStore(
    (store) => store.activeThreadOrderAnchorById,
  );
  const reorderActiveThreads = useUiStateStore((store) => store.reorderActiveThreads);
  ...
  const {
    ...
    activeThreads: automaticallyOrderedActiveThreads,
    snoozedThreads,
    settledThreads,
    snoozeNow,
  } = useMemo(() => {
  ...
  const activeThreads = useMemo(
    () =>
      applyManualActiveThreadOrder({
        threads: automaticallyOrderedActiveThreads,
        preferredIds: activeThreadOrder,
        anchorById: activeThreadOrderAnchorById,
        getId: (thread) => scopedThreadKey(scopeThreadRef(thread.environmentId, thread.id)),
      }),
    [activeThreadOrder, activeThreadOrderAnchorById, automaticallyOrderedActiveThreads],
  );
    ...
    (event: DragEndEvent) => {
      ...
      const anchorById = Object.fromEntries(
        activeThreads.map((thread, index) => [
          keys[index]!,
          activeSidebarThreadOrderAnchorTimestampMs(thread),
        ]),
      );
      reorderActiveThreads(keys, anchorById, activeKey, overKey);
    },
    [activeThreads, reorderActiveThreads],
  );
              ...
              <ul ref={attachListAutoAnimateRef} role="list" className="flex flex-col gap-px">
                {(() => {
                  ...
                  if (activeThreads.length > 0) {
                    const activeThreadKeys = activeThreads.map((thread) =>
                      scopedThreadKey(scopeThreadRef(thread.environmentId, thread.id)),
                    );
                    items.push(
                      <DndContext
                      ...
                      >
                        <SortableContext
                          items={activeThreadKeys}
                          strategy={verticalListSortingStrategy}
                        >
                          {activeThreads.map((thread, index) => (
                            <SortableThreadRow
                              key={activeThreadKeys[index]!}
                              id={activeThreadKeys[index]!}
                            >
